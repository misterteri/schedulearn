...
from lib import get_docker_client, get_busiest_server
...
def run_job(new_job_id: int, background_tasks: BackgroundTasks):
    with Session(db.engine) as session:
        ...
        if scheduling_algorithm.value == "ElasticFIFO":
            ...
            if found['server'] is not None and len(found['gpus']) >= new_job.required_gpus:
                ...
                busiest_server = get_busiest_server() # e.g. 'gpu3'
                # 2. Find the job with the most number of GPUs
                job_with_most_gpus = session.exec(
                    select(db.Job)
                    .where(col(db.Job.trained_at) == busiest_server)
                    .order_by(col(db.Job.required_gpus).desc())
                ).first()
                # 3. Scale in the job
                scale_in_job(job_with_most_gpus.id)
                # 4. Get the ids of GPUs where the busiest job was running
                released_gpus = job_with_most_gpus.trained_at # e.g. [1, 2, 3]
    ...
    with Session(db.engine) as session:
        ...
        if status.get('StatusCode') == 0:
            ...
            if scheduling_algorithm.value == "ElasticFIFO":
                slowest_unfinished_job = session.exec(
                    select(db.Job)
                    .where(col(db.Job.status) == "Running")
                    .order_by(col(db.Job.started_at).asc())
                ).first()
                scale_out_job(slowest_unfinished_job.id)
